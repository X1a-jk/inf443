...
vec3 cardinal_spline_interpolation(float t, float t0, float t1, float t2, float t3, vec3 const& p0, vec3 const& p1, vec3 const& p2, vec3 const& p3, float K)
{
    ...
    float s = (t - t1) / (t2 - t1);
    vec3 d0 = 2.0 * K * (p2 - p0) / (t2 - t0);
    vec3 d1 = 2.0 * K * (p3 - p1) / (t3 - t1);
    vec3 const p = (2 * s * s * s - 3 * s * s + 1) * p1 + (s * s * s - 2 * s * s + s) * d0 + (-2 * s * s * s + 3 * s * s) * p2 + (s * s * s - s * s) * d1;
    return p;
}
